...
def get_PI_bootstrap(
    ...
    beta_matrix = np.array(model.params)[:-1] # exclude sigma^2 - GARCH
    ...
    for i in range(B):
        ...
        new_model = ARIMA(
            endog=new_sub_train_df.Price_MWh,
            exog=new_sub_train_df.drop('Price_MWh',1),
        ).fit(method='yule_walker')

        new_forecast = new_model.forecast(exog=forecast_exogs.loc[hour]).values[0]
        PI_bootstrap.append(new_forecast)
...
def get_forecast_AR(
    ...
    model_type='AR', PI_calculate=False, bootstrap_B=200
):
    ...
    for forecast_day in PI_construction_dates:
        ...
        for hour in range(24):
            ...
            model = ARIMA(
                endog=sub_train_df['Price_MWh'],
                exog=sub_train_df.drop('Price_MWh', axis=1),
            ).fit(method='yule_walker')
            # One-step ahead forecast
            temp_forecast_df = model.forecast(exog=forecast_exogs.loc[hour]) \
            .to_frame('Forecast')
            if PI_calculate:
